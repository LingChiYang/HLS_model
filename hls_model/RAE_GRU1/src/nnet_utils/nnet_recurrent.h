...
void gru_static(bool reset_state, 
                data_T data0[CONFIG_T::n_in/2], 
                data_T data1[CONFIG_T::n_in/2], 
                res_T h_newstate[CONFIG_T::n_state],
    ...
    static res_T h_state[CONFIG_T::n_state];
    typename CONFIG_T::accum_dense_t tmpres0[CONFIG_T::n_state * 3];
    typename CONFIG_T::accum_dense_t tmpres1[CONFIG_T::n_state * 3];
    typename CONFIG_T::accum_dense_t tmpres_state_zr0[CONFIG_T::n_state * 3];
    typename CONFIG_T::accum_dense_t tmpres_state_zr1[CONFIG_T::n_state * 3];
    typename CONFIG_T::accum_t tmpres_state_h[CONFIG_T::n_state];
    ...
    typename CONFIG_T::state_t qh_state0[CONFIG_T::n_state/2];
    typename CONFIG_T::state_t qh_state1[CONFIG_T::n_state/2];
    
    #pragma HLS ARRAY_PARTITION variable=qh_state0         complete
    #pragma HLS ARRAY_PARTITION variable=qh_state1         complete
    //#pragma HLS ARRAY_PARTITION variable=h_state         complete
    //#pragma HLS ARRAY_PARTITION variable=h_newstate      complete
    //#pragma HLS ARRAY_PARTITION variable=tmpres0          complete
    //#pragma HLS ARRAY_PARTITION variable=tmpres1          complete
    //#pragma HLS ARRAY_PARTITION variable=tmpres_state_zr0 complete
    //#pragma HLS ARRAY_PARTITION variable=tmpres_state_zr1 complete
    //#pragma HLS ARRAY_PARTITION variable=tmpres_state_h  complete
    //#pragma HLS ARRAY_PARTITION variable=tmpres_zr       complete
    //#pragma HLS ARRAY_PARTITION variable=tmpres_h        complete
    //#pragma HLS ARRAY_PARTITION variable=inputacc_zr     complete
    ...
    for (int i_h_state = 0; i_h_state < (CONFIG_T::n_state)/2; i_h_state++) {
        #pragma HLS PIPELINE
        qh_state0[i_h_state] = (typename CONFIG_T::state_t) h_state[i_h_state];
    }

    for (int i_h_state = 0; i_h_state < (CONFIG_T::n_state)/2; i_h_state++) {
        #pragma HLS PIPELINE
        qh_state1[i_h_state] = (typename CONFIG_T::state_t) h_state[i_h_state+(CONFIG_T::n_state)/2];
    }
    //std::cout << "data0[0] = " << data0[0] << std::endl;
    //std::cout << "data1[0] = " << data1[0] << std::endl;
    //std::cout << "param0[0] = " << param0[0] << std::endl;
    //std::cout << "param1[0] = " << param1[0] << std::endl; 
    //std::cout << "param_zr0[0] = " << param_zr0[0] << std::endl;
    //std::cout << "param_zr1[0] = " << param_zr1[0] << std::endl;   
    nnet::dense<data_T, typename CONFIG_T::accum_dense_t, typename CONFIG_T::mult_config1>(data0, tmpres0, param0, param_b);
    nnet::dense<data_T, typename CONFIG_T::accum_dense_t, typename CONFIG_T::mult_config1>(data1, tmpres1, param1, param_b);
    nnet::dense<typename CONFIG_T::state_t, typename CONFIG_T::accum_dense_t, typename CONFIG_T::mult_config2>(qh_state0, tmpres_state_zr0, param_zr0, param_br);
    nnet::dense<typename CONFIG_T::state_t, typename CONFIG_T::accum_dense_t, typename CONFIG_T::mult_config2>(qh_state1, tmpres_state_zr1, param_zr1, param_br);
    // Adding the individual vectors from the multiplication of tmpres = Wx*x(t); tmpres_state_zr = Wh*h(t-1); tmpres
    // initialized with biases -- DONE
    for (int iacc = 0; iacc < (2 * CONFIG_T::n_state); iacc++) {
        #pragma HLS PIPELINE
        int index = iacc;
        inputacc_zr[iacc] = tmpres0[index] + tmpres1[index] + tmpres_state_zr0[index] + tmpres_state_zr1[index];
    }
    ...
    for (int iacc = 0; iacc < (CONFIG_T::n_state); iacc++) {
        #pragma HLS PIPELINE
        tmpres_state_h[iacc] = tmpres_zr[iacc + (CONFIG_T::n_state)] * (tmpres_state_zr0[iacc + (2 * CONFIG_T::n_state)] + tmpres_state_zr1[iacc + (2 * CONFIG_T::n_state)]);
    }
    ...
    for (int iacc = 0; iacc < (CONFIG_T::n_state); iacc++) {
        #pragma HLS PIPELINE
        int index = iacc + CONFIG_T::n_state * 2;
        inputacc_h[iacc] = tmpres0[index] + tmpres1[index] + tmpres_state_h[iacc];
    }
    ...
    for (int iacc = 0; iacc < (CONFIG_T::n_state); iacc++) {
        #pragma HLS PIPELINE
        h_state[iacc] = (res_T)(tmpres_h[iacc] * (1 - tmpres_zr[iacc]) + (qh_state0[iacc] + qh_state1[iacc]) * tmpres_zr[iacc]);
        h_newstate[iacc] = h_state[iacc];
    }
}
